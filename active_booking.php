...
<body>
    ...
    <script>
        ...
        function submitReview() {
            ...
            if (!currentBookingId || !currentDriverId) {
                alert('Invalid booking or driver information');
                return;
            }
        ...
        // Poll for status changes every 5 seconds (reduced frequency)
        setInterval(() => {
            if (<?php echo $booking ? 'true' : 'false'; ?>) {
                fetch('get_booking_status.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ booking_id: <?php echo $booking ? $booking['id'] : 'null'; ?> })
                })
                .then(response => response.json())
                .then(data => {
